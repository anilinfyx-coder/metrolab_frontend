...
export default function PhysicalExamEditForm({ waitingListId, labTestId, patientId, onSuccess, onClose, locked }: { waitingListId: number, labTestId: number, patientId: number, onSuccess: () => void, onClose: () => void, locked: boolean }) {
  ...
  useEffect(() => {
    setLoading(true);
    apiFetch<any>(`/api/PhysicalExaminationCertificates?waiting_list_id=${waitingListId}&lab_test_id=${labTestId}`, { tokenKey: 'admin_token' })
      .then(res => {
        if (Array.isArray(res) && res.length > 0) {
          ...
          setForm({
            ...
            bp: cert.bp || '',
            pulse: cert.pulse || '',
            hearing_right: cert.hearing_right || '',
            hearing_left: cert.hearing_left || '',
            vision_right: cert.vision_right || '',
            vision_left: cert.vision_left || '',
            wear_glasses: cert.wear_glasses || false,
            eval_head: cert.eval_head || 'Normal',
            eval_nose: cert.eval_nose || 'Normal',
            eval_mouth: cert.eval_mouth || 'Normal',
            eval_ears: cert.eval_ears || 'Normal',
            eval_eyes: cert.eval_eyes || 'Normal',
            eval_lungs: cert.eval_lungs || 'Normal',
            eval_heart: cert.eval_heart || 'Normal',
            eval_vascular: cert.eval_vascular || 'Normal',
            eval_abdomen: cert.eval_abdomen || 'Normal',
            eval_spine: cert.eval_spine || 'Normal',
            eval_skin: cert.eval_skin || 'Normal',
            eval_neurologic: cert.eval_neurologic || 'Normal',
            additional_comments: cert.additional_comments || '',
            overall_condition: cert.overall_condition || 'Fit',
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (certId) {
        await apiFetch(`/api/PhysicalExaminationCertificates/${certId}`, {
          method: 'PUT',
          tokenKey: 'admin_token',
          body: JSON.stringify(payload),
          successMessage: 'Physical Examination Certificate updated successfully!',
          errorFallback: 'Failed to update certificate.',
        });
      } else {
        const res = await apiFetch<any>(`/api/PhysicalExaminationCertificates`, {
          method: 'POST',
          tokenKey: 'admin_token',
          body: JSON.stringify(payload),
          successMessage: 'Physical Examination Certificate created successfully!',
    ...
    <div className="card-body wl-test-form-body" style={{ padding: '2rem' }}>
      ...
      <div className="cert-form-section">
        ...
        <div className="cert-form-grid cert-form-grid-3">
          ...
          <FormGroup label="Examining Clinician Name" htmlFor="clinician_name" required>
            <input
              id="clinician_name"
              type="text"
              className="form-control"
