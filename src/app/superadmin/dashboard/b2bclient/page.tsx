...
const emptyClient = {
  ...
  website: '', medical_officer_name: '', mrocc: '', clia_number: '',
  smtp_server: '', smtp_port: '', smtp_email: '', smtp_password: '',
  tagline: '', primary_color_code: '', approval_note: '', password: '',
};

type View = 'list' | 'form' | 'subscription' | 'labtestaccess';
...
export default function B2BClientsPage() {
  ...
  const saveClient = async () => {
    ...
    const res = await fetch(url, {
      ...
      body: JSON.stringify({ ...form, role_id: 2, status: true, deleted: false })
    });
                      ...
                      <td style={{ padding: '0.75rem 1rem' }}>
                        <div style={{ display: 'flex', gap: '0.4rem', flexWrap: 'wrap' }}>
                          <button title="Subscription" className="btn btn-ghost"
                            style={{ fontSize: '0.72rem', padding: '0.2rem 0.5rem', background: 'rgba(13,138,111,0.2)', color: '#0d8a6f', border: '1px solid rgba(13,138,111,0.3)' }}
