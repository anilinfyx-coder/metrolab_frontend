...
export default function ViewLabTestFormModal({ labTestId, token, onClose }: Props) {
  ...
  return (
    <div className="confirm-modal-overlay" role="presentation" onClick={onClose}>
      ...
      >
        <div className="view-form-modal-header">
          <h2 id="view-form-title">{test?.name || 'View Form'}</h2>
          <button type="button" className="btn btn-ghost" onClick={onClose}>✕ Close</button>
        </div>
        <div className="view-form-modal-body">
          {loading ? (
            <div style={{ padding: '2rem', textAlign: 'center', color: 'var(--text-muted)' }}>Loading form…</div>
          ) : !test ? (
            <div style={{ padding: '2rem', textAlign: 'center', color: 'var(--text-muted)' }}>Test not found.</div>
          ) : (
            <>
              {test.description && (
